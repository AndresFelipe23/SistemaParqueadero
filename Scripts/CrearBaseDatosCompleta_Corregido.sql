...
-- Eliminar base de datos si existe (para recrearla)
IF EXISTS (SELECT * FROM sys.databases WHERE name = 'SistemaParqueadero')
BEGIN
    ALTER DATABASE SistemaParqueadero SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE SistemaParqueadero;
    PRINT 'Base de datos existente eliminada.';
END
GO

-- Crear la base de datos (sin especificar rutas, usa configuración predeterminada)
